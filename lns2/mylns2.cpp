...
MyLns2::MyLns2(int seed, vector<vector<int>> obs_map,vector<pair<int,int>> start_poss, vector<pair<int,int>> goal_poss, int all_ag_num,int row, int column):
        instance(obs_map,start_poss,goal_poss, all_ag_num,row,column),path_table(row*column, all_ag_num),
        collision_graph(all_ag_num),goal_table(row*column, -1)
{
    srand(seed);
...
bool MyLns2::rest_lns(int num_local_ag, double cutoffTime)
{
    start_time = Time::now();
...
bool MyLns2::rest_runPP()
{
    auto rest_shuffled_agents = neighbor.agents;
    std::random_shuffle(rest_shuffled_agents.begin(), rest_shuffled_agents.end());
    auto p = rest_shuffled_agents.begin();
    neighbor.colliding_pairs.clear();
    ConstraintTable constraint_table(instance.num_of_cols, instance.map_size, &path_table);

    while (p != rest_shuffled_agents.end() && ((fsec)(Time::now() - start_time)).count() < time_limit)
    {
        int id = *p;
        agents[id].path = agents[id].path_planner->findPath(constraint_table);
        if (agents[id].path_planner->num_collisions > 0)
    ...
    if (p == rest_shuffled_agents.end() && neighbor.colliding_pairs.size() <= neighbor.old_colliding_pairs.size()) // accept new paths
    {
        return true;
    }
    else // stick to old paths
    {
        auto p2 = rest_shuffled_agents.begin();
        while (p2 != p)
        {
            int a = *p2;
...
void MyLns2::add_sipps(vector<vector<pair<int,int>>> temp_path,vector<int> selected_ag,const vector<pair<int,int>>& vector_start_locations)
{
